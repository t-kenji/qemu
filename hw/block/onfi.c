/*
 * Open Nand Flash Interface emulation.
 * Based on ONFI Rev4.2 and "4Gx1bit NAND Flash Memory" datasheet for 
 * the MT29F4G01ABBFD12 chips from Micron.
 *
...
enum {
    ONFI_CMD__READ = 0x00,
    ONFI_CMD__CHANGE_READ_COL = 0x05,
    ONFI_CMD__BLOCK_ERASE = 0x60,
    ONFI_CMD__READ_STATUS = 0x70,
    ONFI_CMD__PAGE_PROGRAM = 0x80,
    ONFI_CMD__CHANGE_WRITE_COL = 0x85,
    ONFI_CMD__READ_ID = 0x90,
    ONFI_CMD__READ_PARAM_PAGE = 0xEC,
    ONFI_CMD__RESET = 0xFF,
};
...
/* Parameter Page Data based on u-boot/include/linux/mtd/rawnand.h */
/* FIXME: want to include from external file */
typedef union {
    uint8_t raw[256];
    struct {
        ...
        uint16_t opt_cmd;
        uint8_t res10_11[2];
        uint16_t ext_param_page_length;
...
#define TYPE_ONFI "onfi"

#define ONFI(obj) \
    OBJECT_CHECK(NANDFlashState, (obj), TYPE_ONFI)
...
static void nand_reset(DeviceState *dev)
{
    ...
    NANDParamPageData reset_data = {
        .sig = {'O','N','F','I'},
        .revision = 0x0200,
        .opt_cmd = 0x0600,
        .manufacturer = {'Q','e','m','u',' ',' ',' ',' ',' ',' ',' ',' '},
        .model = {'Q','e','m','u','-','1','2','3','4',' ',
                  ' ',' ',' ',' ',' ',' ',' ',' ',' ',' '},
        .jedec_id = s->manf_id,
        .data_bytes_per_page = 4 * KiB,
        .spare_bytes_per_page = 256,
        .data_bytes_per_ppage = 1 * KiB,
        .spare_bytes_per_ppage = 64,
        .pages_per_block = 64,
        .blocks_per_lun = 2048,
        .lun_count = 1,
        .addr_cycles = 0,
        .bits_per_cell = 1,
        .bb_per_lun = 40,
        .block_endurance = 1281,
        .guaranteed_good_blocks = 8,
        .guaranteed_block_endurance = 0,
        .programs_per_page = 4,
        .ecc_bits = 8,
    };
    s->params = reset_data;
    s->params.crc = onfi_crc16(0x4F4E, s->params.raw,
                               sizeof(s->params.raw) - sizeof(s->params.crc));
}
...
static void nand_command(NANDFlashState *s)
{
    ...
    case ONFI_CMD__READ_ID:
        ...
        if (s->addr == 0x00) {
            ...
            hw_error("%s: Unsupported addr %#" PRIx64 "\n",
                     __func__, s->addr);
        }
        break;

    case ONFI_CMD__READ_PARAM_PAGE:
        s->ioaddr = s->io;
        s->iolen = 0;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < sizeof(s->params.raw); j++) {
                nand_pushio_byte(s, s->params.raw[j]);
            }
        }
        break;
...
void onfi_setio(DeviceState *dev, uint32_t value)
{
    ...
    if (s->ale) {
        ...
        switch (s->addrlen) {
        case 1:
            if ((s->cmd == ONFI_CMD__READ_ID)
                || (s->cmd == ONFI_CMD__READ_PARAM_PAGE)) {
                nand_command(s);
            }
            break;
...
DeviceState *onfi_init(BlockBackend *blk, int manf_id, int chip_id)
{
    ...
    dev = DEVICE(object_new(TYPE_ONFI));
