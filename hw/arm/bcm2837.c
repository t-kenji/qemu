/*
 * Raspberry Pi emulation (c) 2017 t-kenji
 *
...
/* "QA53" (Pi3) interrupt controller and mailboxes etc. */
#define BCM2837_CONTROL_BASE    0x40000000
...
static void bcm2837_realize(DeviceState *dev, Error **errp)
{
    ...
    /* bcm2837 interrupt controller (and mailboxes, etc.) */
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(&s->control), 0, BCM2837_CONTROL_BASE);
    ...
    for (n = 0; n < BCM2836_NCPUS; n++) {
        ...
        object_property_set_int(OBJECT(&s->cpus[n]),
                                BCM2837_PERI_BASE + MCORE_OFFSET,
                                "reset-cbar", &err);
        if (err) {
            error_propagate(errp, err);
            return;
