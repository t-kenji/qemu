/*
 * Freescale T1 SoC Peripherals definitions.
 *
...
static const RegDef32 t102x_cpc_regs[] = {
    ...
    REG_ITEM(CPC_CPCSRCR0,      0x0104, 0xFFFC0007, 0xFFFFFFFF), /* set by PBL */
...
static void t102x_cpc_map_sram(T102xCPCState *s)
{
    hwaddr ba = s->regs[CPC_CPCSRCR0] & 0xFFFF8000;
    ba |= (hwaddr)s->regs[CPC_CPCSRCR1] << 32;

    if (s->sram.container == NULL) {
        memory_region_add_subregion(get_system_memory(), ba, &s->sram);
    } else {
...
static void t102x_cpc_realize(DeviceState *dev, Error **errp)
{
    ...
    memory_region_init_ram(&s->sram, OBJECT(s), "cpc-sram", 256 * KiB, errp);
